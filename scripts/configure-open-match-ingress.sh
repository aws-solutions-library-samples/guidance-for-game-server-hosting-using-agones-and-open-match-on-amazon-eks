...
kubectl expose deployment open-match-frontend  -n open-match  --type=LoadBalancer  --name=open-match-frontend-loadbalancer -o yaml --dry-run=client | sed  's/creationTimestamp: null/annotations:\n    service\.beta\.kubernetes\.io\/aws-load-balancer-name: "'${CLUSTER_NAME}'-om-fe"/g' | kubectl apply -f -
